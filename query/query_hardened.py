...
class QuerySystem:
    ...
    def __init__(self, base_path: Optional[str] = None, debug: bool = False, timeout: int = DEFAULT_TIMEOUT):
        """
        ...
        if self.debug:
            print(f"[DEBUG] QuerySystem initialized", file=sys.stderr)
            print(f"[DEBUG] Base path: {self.base_path}", file=sys.stderr)
            print(f"[DEBUG] DB path: {self.db_path}", file=sys.stderr)
            print(f"[DEBUG] Timeout: {self.timeout}s", file=sys.stderr)
            print(f"[DEBUG] Readonly mode: {self._readonly_mode}", file=sys.stderr)
    ...
    def _init_database(self):
        ...
        # Wrap in try/except for readonly databases
        try:
