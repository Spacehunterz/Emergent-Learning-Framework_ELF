#!/usr/bin/env python3
"""
Pre-Tool Learning Hook: Auto-inject relevant heuristics before investigation.

This hook closes the learning loop by:
1. Auto-querying the building for relevant heuristics
2. Injecting applicable rules into the agent's context
3. Tracking which heuristics are being consulted (for validation)
...
def format_learning_context(heuristics: List[Dict], failures: List[Dict], domains: List[str]) -> str:
...
def main():
    ...
    # If we have learning context, inject it
    if heuristics or failures:
        learning_context = format_learning_context(heuristics, failures, domains)
